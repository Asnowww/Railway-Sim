...
const trains = ref<TrainMonitorState[]>([
  { id: 'T101', serviceNo: 'G101', positionPercent: 12, speedKph: 62, mode: 'ATO', loadRate: 46, faultCode: '', section: '上京南-科技园' },
  { id: 'T203', serviceNo: 'G203', positionPercent: 31, speedKph: 71, mode: 'ATO', loadRate: 83, faultCode: '', section: '科技园-人民广场' },
  { id: 'T305', serviceNo: 'G305', positionPercent: 54, speedKph: 0, mode: 'ATP', loadRate: 108, faultCode: 'VHL-23', section: '人民广场-金融城' },
  { id: 'T407', serviceNo: 'G407', positionPercent: 76, speedKph: 58, mode: '人工', loadRate: 69, faultCode: '', section: '金融城-会展中心' },
  { id: 'T509', serviceNo: 'G509', positionPercent: 89, speedKph: 44, mode: 'ATO', loadRate: 37, faultCode: '', section: '会展中心-机场北' }
])
...
const powerSections = ref<PowerSectionState[]>([
  { id: 'P1', name: '牵引一区', startPercent: 4, widthPercent: 28, voltage: 1510, current: 810, status: 'ENERGIZED', affectedTrains: [] },
  { id: 'P2', name: '牵引二区', startPercent: 32, widthPercent: 28, voltage: 1680, current: 1280, status: 'OVERRANGE', affectedTrains: ['T305'] },
  { id: 'P3', name: '牵引三区', startPercent: 60, widthPercent: 35, voltage: 0, current: 0, status: 'LOST', affectedTrains: ['T407', 'T509'] }
])
...
const overloadedStations = computed(() => passengerStations.value.filter((station) => station.waiting / station.capacity >= 0.8))
...
const filteredAlarms = computed(() => {
  ...
  if (alarmFilter.value === '未确认') {
    return alarmList.filter((alarm) => !alarm.confirmed)
  }
...
function focusAlarm(alarm: UnifiedAlarm): void {
  selectedLocation.value = stations.includes(alarm.location) ? alarm.location : '人民广场'
}

function updateThresholds(): void {
  trainCrowdThreshold.value = sanitizeNumericInput(trainCrowdThreshold.value, 50, 100)
  stationCrowdThreshold.value = sanitizeNumericInput(stationCrowdThreshold.value, 50, 100)
  predictionWindowMinutes.value = sanitizeNumericInput(predictionWindowMinutes.value, 5, 30)
}
...
watch([trainCrowdThreshold, stationCrowdThreshold, predictionWindowMinutes], updateThresholds)
  ...
  <main class="monitor-shell">
    <header class="topbar">
      ...
      <section class="topbar-actions" aria-label="仿真状态">
        <span class="status-pill running">仿真同步</span>
    ...
    <section class="lower-grid">
      ...
      <section class="panel settings-panel">
        ...
        <p>敏感配置变更已做范围校验，正式接入后应由后端记录操作日志并二次确认。</p>
      </section>
    </section>
  </main>
</template>
