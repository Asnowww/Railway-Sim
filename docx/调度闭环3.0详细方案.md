...
## 21. 闭环完善实施补充（2026-07-12）

本阶段在 3.0 本车调节模型之上完成了以下工程闭环。
...
计划加载器将其转换为 `CirculationPlan`、`TrainServicePlan` 和 `PlannedStop`，同时保留旧的仅按时段间隔推算计划作为兼容降级路径。

### 21.2 调度生成正式发车指令

旧的运行时“遍历每个车站并临时造车”逻辑已停止调用。现在由 `DispatchService` 根据正式车次的始发站计划时刻生成 `DEPART`：

```json
{
  "source": "FORMAL_SERVICE_PLAN",
  "serviceId": "SVC-003",
  "circulationId": "CIRC-003",
  "trainNo": 3,
  "fromStation": "S01",
  "toStation": "S05",
  "direction": "DOWN",
...
运行时只负责消费发车指令、创建列车并反馈 `TRAIN_CREATED`、`TRAIN_ALREADY_ACTIVE`、`DEPART_PAYLOAD_MISSING`、`DEPART_TRAIN_NO_INVALID` 或 `TRAIN_CREATION_FAILED`。

### 21.3 真正的车站级间隔观测

新的观测对象为 `StationHeadwayObservation`，严格按照：
...
计算实际间隔。每条车站事件同时记录 `serviceId`、`circulationId` 和 `direction`。位置相邻列车推算只在没有车站级事件时作为兼容降级，不再是主要间隔来源。

### 21.4 调度策略与效果确认

停站调整量根据超限秒数分级，单次限制在 3—10 秒：
...
命令保存基准间隔误差和基准晚点。时间间隔命令的 `EFFECT_CONFIRMED` 标准为：

1. 必须取得命令创建时刻之后的新车站发车观测；
2. 新间隔进入目标容差；或
3. 新的绝对间隔误差至少改善 `max(5 秒, 基准绝对误差 × 25%)`；
4. 无前车参考的首车晚点恢复，要求后续实际晚点至少改善配置的最小秒数。

“已经开始执行停站调整”只进入 `APPLIED`，不能直接等同于效果确认。
...
| 错误 | 分类 | 是否重试 |
|---|---|---|
| 进路占用、冲突、锁闭资源忙 | `RESOURCE_CONFLICT` | 是 |
| 道岔暂不可用 | `SWITCH_STATE` | 是 |
| 临时忙或通信超时 | `TEMPORARY/TIMEOUT` | 是 |
| 找不到进路或配置缺失 | `CONFIGURATION` | 否 |
| 请求参数或命令类型无效 | `INVALID_REQUEST` | 否 |
| 列车已经进入后请求取消 | `CANCEL_NOT_ALLOWED` | 否 |

不可重试错误会阻止同一列车、同一进路继续自动重发；可重试错误遵守冷却时间和最大重试次数。
...
effectivePriority
= basePriority
+ min(maxWaitingBonus, waitingSeconds × waitingPriorityPerSecond)
```

等待中的列车保留首次等待时刻；已选中的列车清除本轮等待累计。该机制使持续等待列车能够在后续仲裁中反超仅因距离更近而长期占优的列车。

### 21.7 进路取消和调度侧超时

进路取消已经打通到联锁：

- 人工取消已接受的路线请求时，调度生成真实 `CANCEL_ROUTE`，不再只修改本地命令状态。
- 联锁取消成功后，预约和决策进入 `CANCELLED`。
- 联锁拒绝取消时，保留原预约状态并记录结构化失败原因。
- 路线请求超过应答期限后进入 `TIMEOUT`，按照可重试规则处理。
- 已建立进路超过持有期限仍未释放时，调度自动生成 `CANCEL_ROUTE`。
- 若列车已经进入进路，联锁拒绝取消并返回不可重试的 `CANCEL_NOT_ALLOWED`。

前端新增正式车次/交路列表、车站级间隔观测面板和进路状态闭环面板，可查看重试次数、错误码、错误类别、下次可重试时间、等待秒数、超时时刻和取消命令 ID。
