...
因此需要在调度模块中补充“道岔与进路调度”能力。本文中“道岔调度”统一落地为“进路级调度”：调度模块选择或建议列车下一条运行进路，信号/联锁模块负责安全办理进路，道岔实际转换和锁闭由轨道/联锁能力完成，车辆模块只按信号授权运行。
...
| 不重构完整运行图 | 第一期只做下一进路选择和局部冲突仲裁，不做全日运行图自动重排。 |
...
SimulationRuntime.tick()
  ...
  4. DispatchService.evaluate()                     时间调度 + 进路调度决策
  5. RouteInterlockingService.applyDispatchCommand() 办理 REQUEST_ROUTE/REROUTE
...
4. 当列车进入道岔调度窗口时，调度生成 REQUEST_ROUTE 或 REROUTE。
...
```text
1. 检测到原计划进路 R-main-down 不可用。
2. 查询替代进路 R-branch-down。
3. 调度生成 REROUTE。
4. 联锁接受：执行改路，并在 dispatch 快照记录新 routeId。
5. 联锁拒绝：对列车生成 HOLD，等待人工或后续恢复。
```
...
表示列车的运营运行意图，由运行计划、车次、目的地、人工命令或扰动策略生成。
...
3. 运行计划或 service pattern 指定线路方向。
...
- 支持人工提交 `REQUEST_ROUTE` 或结构化 `REROUTE`。
- 联锁返回接受/拒绝结果。
...
- 根据 `serviceNo`、`destinationStationId` 或配置映射 route。
- 列车接近道岔窗口时自动申请目标 route。
- 同一列车同一路口增加冷却，避免重复申请。
...
道岔调度不应设计为调度模块直接控制道岔，而应设计为运营层的进路选择与冲突仲裁能力。调度负责判断列车应该走哪条进路、冲突时谁先通过、谁等待或限速；信号/联锁负责检查安全条件、建立进路、锁闭道岔并生成 MA；轨道模块负责实际区段和道岔状态；车辆模块只按最终信号授权和车载控制逻辑运行。

第一期建议以“人工进路请求 + 自动冲突等待”为最小闭环，随后扩展到分岔自动选路、合流优先级仲裁和故障改路。该设计能与现有时间维度调度自然结合，并为后续外部车辆控制服务拆分保持清晰边界。
