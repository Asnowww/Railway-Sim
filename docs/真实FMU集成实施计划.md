...
| 9000 FMU 服务 | `BaseHTTPRequestHandler + SimpleFallbackModel`；`FleetStepper` 仅缓存 Python 包装实例；无 FMPy | **[待实施]** | FastAPI + FMPy，持久 `FMU2Slave`，一车一实例，完整生命周期和批量步进。 |
| 9200 供电仿真 | FastAPI；按分区聚合牵引/再生负荷，计算网压与`powerAvailableWatts` | **[已完成底座]** | 增加`regenPowerAvailableWatts`，保持受流功率和回馈功率为电网侧口径。 |
...
| 总质量传入车辆物理模型 | 9300 `VehicleLoadPolicy` 和 `VehiclePhysicsInputDto.trainMassKg` | **[已完成底座]** | WP1 将硬编码改成真实 YAML 加载；质量继续每步输入。 |
| 车辆参数 YAML | `config/train_params.yaml` | **[已存在]** | WP1 将其变成 9300/9000 的真实参数事实来源。 |
| Modelica 模型草案 | `fmu-service/modelica/TrainTractionBrake.mo` | **[待修正]** | WP2 修正类型、功率口径、状态、再生限制和参数加载。 |
| FMU 映射草案 | `config/fmu_mapping.yaml` | **[待修正]** | WP0/WP2 补充生命周期、变量语义、机械/电功率和再生能力。 |
| 真实 `.fmu` | 仓库中不存在导出的 FMU | **[待实施]** | WP3。 |
| FMPy 依赖和实例 | 9000 无依赖清单、无 `fmpy` 导入 | **[待实施]** | WP4。 |
| 一份 FMU、多实例 | `FleetStepper` 有按`trainId`缓存的骨架，但实际调用 Python fallback | **[待实施]** | WP4。 |
...
| 再生吸收能力约束 | 9200 只有`regenAvailable`布尔量 | **[待实施]** | WP6 增加瓦特级能力并贯通 DTO/FMU。 |

### 2.3 已确认的差异与必须修正项

1. 现有计划将 100 ms 写成待实施事项，实际已经完成；后续只做实时性验证。
2. 9300 当前只是用硬编码复制 YAML 中的 `198000/72000 kg`，并未读取 YAML。
3. 现有 Modelica 草案把 Boolean 输入建成 Real，把故障码建成 Real，不满足目标契约。
4. 现有 Modelica 草案用`powerAvailable/maxTractionPower`直接缩放牵引力，混淆了电网侧功率和电机侧机械功率。
5. `traction.maxPowerWatts=3200000`的确定含义是**电机侧最大机械功率**，不是受流侧电功率。
6. 现有 9200 只输出`powerAvailableWatts`和`regenAvailable`，无法给 FMU 提供瓦特级再生吸收上限。
7. 正常 STEP 不得每步用中央传来的位置、速度覆盖 FMU 内部状态；但总质量会在车站上下客后改变，因此`trainMassKg`必须是每步输入。
...
- **[已完成底座]** Modelica草案、FMU映射草案、9000一车一包装实例骨架。
- **[待实施]** YAML真实参数统一、修正后的Modelica模型、可复现FMU导出、FMPy `FMU2Slave`、9000 FastAPI生命周期、9300批量物理执行器、再生功率约束、多车闭环和系统验收。

因此，当前项目仍不能宣称“真实车辆FMU已经接入”，只能宣称“真实FMU集成方案和可执行实施基线已经完成”。
