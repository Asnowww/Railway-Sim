...
> **当前多周期契约（2026-07-14）：**9300 每 100 ms 计算一次 TCMS 控制决策，在该周期内对控制量和安全约束采样保持，连续调用 9000 执行 5 个 20 ms FMU 物理子步，然后仅把第 5 个子步的最终状态提交给 9300 并回传 8080。文中历史 WP 记录中的单次 100 ms FMU 数据仅作为旧基线，不再代表当前契约或当前性能验收结论。
...
| 最新车辆参数曲线升级 | UP0～UP6 | [05-最新车辆参数曲线升级计划](真实FMU集成实施计划/05-最新车辆参数曲线升级计划-UP0-UP6.md) |

> 2026-07-11补充：WP0～WP8记录的是标量车辆模型闭环的既有实施和验收历史。最新车辆资料引入52点电机特性曲线、六车编组质量及动态Davis阻力，后续升级按UP0～UP6执行；不得通过回改既有验收记录掩盖模型代际差异。
...
8080 中央系统
  产生 MA、信号、轨道、调度约束并镜像车辆状态
        ...
        v
9300 车辆运行服务
  车辆状态机每 100 ms 产生牵引/制动指令，并在周期内保持
        |
        v
9000 FMU 执行服务
  Python + FMPy，一份 FMI 2.0 CS FMU、多辆列车独立实例，每 20 ms 一个物理子步
        |
        v
...
3. 9300 每 100 ms 控制周期批量调用 9000 五次，每次推进 20 ms，并对每辆列车维持独立实例生命周期。
4. 9200 返回的网压、电网侧牵引可用功率和再生吸收功率进入下一车辆物理步。
5. 两辆同供电段列车通过 9200 发生可测的物理耦合。
6. WP8 中车辆仿真部署范围内的接口、物理、性能、降级和回归验收全部通过。

当前第1～6项已经完成；在本轮约定的9000/9200/9300车辆仿真部署边界内，可以表述为“真实FMU车电闭环已经完成部署与系统验收”。8080、MySQL和演示前端仍由其原部署与维护边界负责。
...
| 9300 车辆运行服务 | Spring Boot；全车Prepare后一次调用9000，统一Apply并写入9200；按车维持FMU/Java粘滞降级状态 | **[已完成WP5、WP7～WP8]** | 已完成容器编排、跨进程故障演示和20车600秒长稳验收。 |
...
| 9200 供电仿真 | FastAPI；按分区聚合电网侧牵引/再生负荷，计算网压、`powerAvailableWatts`、再生预算和未吸收回馈 | **[已完成WP6、WP7～WP8]** | 启动直接加载1500 V、5分区YAML，已纳入统一健康、启动顺序和连续闭环验收。 |
...
1. 扩展`docker-compose.yml`，只编排本轮修改的9000、9200和9300并加入健康检查；8080中央系统与MySQL继续使用既有部署和接口，不纳入本编排。
2. 9000挂载或复制FMU制品和车辆YAML；9300挂载同一车辆YAML。
3. 启动依赖固定为：9200和9000健康后启动9300；既有8080由中央系统部署方配置9300/9200地址并自行管理启动。
4. 增加超时、重试上限和停止顺序；本编排停止时先停9300，再停9000/9200。
...
python3 scripts/test-deployment-recovery.py --env-file deploy/server.env
...
- **[验收]** 全新环境按Compose可一次启动9000、9200和9300并通过三服务健康检查。
...
4. 以兼容回归方式验证现有中央接口；确认`App.vue`未修改并记录前端仓库既有缺失源文件导致的构建基线，不把8080、MySQL或前端加入本轮部署制品。
...
- **[已完成]** WP7三项仿真服务容器编排、健康与系统级故障策略，以及WP8持续运行、性能和仿真侧最终回归验收。

因此，当前可以宣称“9300生产车辆循环已经接入真实FMU，与9200形成100 ms显式多车车电闭环，并已完成仿真侧WP7～WP8部署、故障恢复和20车600秒性能验收”。8080中央和MySQL沿用既有部署；前端未修改`App.vue`，其仓库既有构建缺口不纳入本轮车辆仿真部署结论。
