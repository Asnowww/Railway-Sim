...
public class PeripheralFrameCodec {
    ...
    public byte[] encode(PeripheralFrame frame) {
        ...
        if (trainId.length > 64) {
            throw new IllegalArgumentException("trainId exceeds 64 UTF-8 bytes");
        }
        if (payload.length > MAX_PAYLOAD_BYTES) {
            throw new IllegalArgumentException("peripheral payload is too large");
        }
        ...
        buffer.put((byte) frame.channel().code());
        buffer.putShort((short) FIXED_HEADER_BYTES);
        buffer.putInt(frame.sequence());
        buffer.putShort((short) trainId.length);
        buffer.putInt(payload.length);
        buffer.putShort((short) frame.flags());
        buffer.put(trainId);
        buffer.put(payload);
        buffer.putInt((int) crc32(buffer.array(), withoutCrc));
        return buffer.array();
    }
    ...
    public PeripheralFrame decode(byte[] bytes) {
        ...
        if (version != VERSION) {
            throw new IllegalArgumentException("unsupported aggregated peripheral version: " + version);
        }
        int headerLength = Short.toUnsignedInt(buffer.getShort(6));
        if (headerLength != FIXED_HEADER_BYTES) {
        ...
        if (bytes.length != expectedLength) {
            throw new IllegalArgumentException("aggregated peripheral total length is invalid");
        }
        long expectedCrc = Integer.toUnsignedLong(buffer.getInt(bytes.length - CRC_BYTES));
        long actualCrc = crc32(bytes, bytes.length - CRC_BYTES);
        ...
        return new PeripheralFrame(
            PeripheralChannel.fromCode(Byte.toUnsignedInt(buffer.get(5))),
            buffer.getInt(8),
            Short.toUnsignedInt(buffer.getShort(18)),
            new String(trainId, StandardCharsets.UTF_8),
            payload
        );
    }
    ...
    private void requireMagic(byte[] bytes) {
        for (int index = 0; index < MAGIC.length; index++) {
            if (bytes[index] != MAGIC[index]) {
                throw new IllegalArgumentException("aggregated peripheral magic is invalid");
            }
        }
    }
